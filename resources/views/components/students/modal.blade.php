...
<div class="modal__dialog">
	...
	<div class="modal__container">
		<div class="modal__content">
			...
			<form id="create-student-form" class="form" method="POST" action="{{ route('students.create') }}">
					...
					<div class="form__grid">
						...
						<div class="form-group">
							...
							<input type="text" id="number_in_the_book"
											class="w-full rounded-md border border-gray-300 px-4 py-2 outline-none focus:border-transparent focus:ring-2 focus:ring-blue-500"
											name="number_in_the_book">
						</div>
						<div class="form-group">
							<label for="status">Ngành đào tạo</label>
							<select id="status"
											class="w-full rounded-md border border-gray-300 px-4 py-2 outline-none focus:border-transparent focus:ring-2 focus:ring-blue-500"
											name="training_id">
								@foreach($trainings as $training)
									<option value="{{ $training->id }}" {{ old('$training_id', '') == $training->id ? 'selected' : '' }}>{{ $training->name }}</option>
								@endforeach
							</select>
						</div>
						<div class="form-group">
				...
				<div class="modal__footer">
					<button type="submit" class="modal__footer--action-primary" onclick="closeModal()">
						Lưu
					</button>
